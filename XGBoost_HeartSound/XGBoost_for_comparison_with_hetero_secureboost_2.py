...
train_X, test_X = train.iloc[:, 2:], test.iloc[:, 2:]
...
model = xgb.XGBClassifier(tree_method='hist', max_depth=3, n_estimators=30, learning_rate=0.3,reg_lambda=0.1,
                          reg_alpha=0, subsample=1.0, min_child_weight=0, max_bin=32, seed=1)

eval_set = [(train_X, train_y), (train_X, train_y)]
XGB = model.fit(train_X, train_y, eval_set=eval_set, verbose=True)
...
print("传统xgboost，医院b，参数大多与纵向secureboost保持一致，测试集（30%）的结果如下：")
print("敏感度 = ", recall1)
print("特异性 = ", recall0)
print("UAR = ", UAR)
print("UF1 = ", UF1)
